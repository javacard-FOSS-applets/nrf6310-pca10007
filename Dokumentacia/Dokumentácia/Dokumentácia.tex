...
\vspace*{18cm}
\section*{Poďakovanie}
Rád by som sa poďakoval vedúcemu diplomovej práce a jeho kolegom, z firmy R-DAS, za poskytnuté technológie a zariadenia, prejavenú dôveru a za odbornú pomoc pri riešení technických problémov, pri vypracovaní záverečného projektu.
...
V implementácií bol použitý príklad od Nordic-u, ktorý jednosmerne posielal dáta pomocou broatcastov. Tento príklad bol rozšírený a doplnený od ďalšie funckionality.
...
\section{SW AES}
Bola použitá online dostupná knižnica na GitHube s názvom TinyAES 128 C.
PSK
Vector
...
odpoveda na nasledujuce apdu spravy v datovej cast. Na parametroch apdu P1 a P2 nezalezi.
\begin{itemize}
...
Princíp automatického testu, spočiva v tom, aby sa zabezpečili konzistentné podienky pri rôznych typoch zabezpečenia.

Počas automatickeho testovania, Master zariadenie neprijíma. Pokial neprijimne data do isteho casoveho okamziku, alebo prijmne ine data ako poslal v posledom, pokusi sa poslat data znovu. 

Test sa ukonci po uspesnom poslani a prijati hodnot, nastaveni parametrom TEST\_MAX\_VALUE.

\section{Dosky plošných spojov}
\subsection{Test board pre ISO7816}

Prvá narvhnutá a realizovaná doska, slúžila v prvom rade na odladenie komunikacie UART na standard iso7816 a na pozorovanie spravanie sa komunikacie s roznym typom pripojenia UART ku kryptoelementu

Konektor vlavo sluzi na pripojenie pinov procesora. Na konektor vprava sa pripojuje samotny kryptoelement.

Prvy jumper umoznuje 
Pomocou jumpra umoznuje odpojit alebo pripojit dva typy pullup rezistorov na I/O linku. Hodnoty osaden=ych rezistorov su 10k a 3k3. 
Vascina literatury tykajuca sa sa komynunikacie so smart kartami odporuca mat pripojeny na komunikacu linku pull up odporu.
Pomocou tychto jumprov je mozne testovat rozne kombinacie zapojeni. Vo vyslednom zapojeni a vprevadze \ref{f:isouart}. Boli pridane tsetpoin pre meranie multimetrom a pripojenie oscliloskopui
...
\subsection{Slave board}
	...
	\ref{f:i_board2}
Pripojenie 
Pripojenie UART a ISO \ref{f:isouart}
vlastny konektor 
nazov z datashetu pre dany konektor vyplynuli.
1.27 mm
pruzne vodice z eternetoveho kabla
V domacich podmienkach podleptanie alebo nedostatocne odleptanie medenej vrstvy. Vo vyvojovom nastroji Eagle bola vytoverena aj vlastna kniznica pre dany konnetkro, kroa sluzi len ako voditko, kde maju byt odvrtane diery a . Vodice su ulozene volne. Takymto sposobom je privedene napajanie a zem k doske spolu s vyhladzujucim kondenzatorom. Najidealnejsie pasovali do protichodneho konektora na Nordic doske jednozilove medene vodice z eternetoveho kabla. Detail zapojenie a pripojenia \ref{f:i_board2}

Dalej su to piny pre na porte0. UART, CLK, VCC, . pripojene ku konekoru.
...
\chapter{Technický manuál} \label{s_manual}
 ...
 \begin{enumerate}
 	...
 	\item otvorte Keil IDE a doinštalujte alebo aktualizujte balíčky, ktoré by ste chceli používať (Nordic dosky, ARM kompilátor),
