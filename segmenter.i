...
uint8_t AddMessage(uint8_t* p_event_message_buffer){
	...
	segment_start = p_event_message_buffer[1+1+1];
	segment_end = p_event_message_buffer[1+1+1+1];
	...
	{
		recieve.message[recieve.count++] = p_event_message_buffer[1+1+1+i+2];
		thisCount++;
	}
...
void SendData(){
	...
	buffer[1+1+1]=send_counter;
	buffer[1+1+1+1]=send_counter + transmit.length - transmit.count;
	...
	else
	{
		...
		{
			buffer[1+1+1+2+i] = transmit.message[transmit.count++];
			send_counter++;
		}
	}
